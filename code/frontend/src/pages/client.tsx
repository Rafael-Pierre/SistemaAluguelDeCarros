...
const App: React.FC = () => {
  ...
  const onChangeDataPickerStart: DatePickerProps["onChange"] = (
    date,
    dateString
  ) => {
  ...
  const onChangeDataPickerEnd: DatePickerProps["onChange"] = (
    date,
    dateString
  ) => {
  ...
  useEffect(() => {
    async function fetchCars() {
      try {
        ...
        const response = await axios.get("http://localhost:5500/car", {
          headers,
        });
      ...
      <div className="cards-client">
        ...
        <Space direction="vertical" size={16}>
          {requestInfo.map((reqInfo) => (
            ...
            >
              <p>
                <strong>Início:</strong> {reqInfo.start}
              </p>
              <p>
                <strong>Fim:</strong> {reqInfo.end}
              </p>
              <p>
                <strong>Valor total:</strong> R$ {reqInfo.total_value}
              </p>
              <p>
                <strong>Carro escolhido:</strong> {reqInfo.car.model}
              </p>
              <p>
                <strong>Ano do carro:</strong> {reqInfo.car.year}
              </p>
              <Button
                style={{
                  color: "white",
                  backgroundColor: getStatusColor(reqInfo.status),
                }}
              >
